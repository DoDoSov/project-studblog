<script>
  ...
  import { authStore, isLoggedIn } from '../../lib/store.js';
  ...
  async function save() {
    saving = true;
    saveError = '';
    // Strip any legacy [github:...] tags that may exist in older content
    const cleanContent = content.replace(/(<p[^>]*>)?\[github:\s*.*?\](<\/p>)?/g, '').trim();
...
<section class="max-w-7xl mx-auto px-6 pb-20">
  {#if !$isLoggedIn}
    <div class="glass-panel p-20 text-center rounded-[3rem] text-white/50">Please log in to manage your articles.</div>
  {:else}
      ...
      <form class="lg:col-span-2 glass-panel p-10 rounded-[2.5rem] space-y-8" on:submit|preventDefault={save}>
        ...
        <div class="flex justify-between items-end">
          <div>
            ...
            <h1 class="text-4xl font-black text-white uppercase tracking-tighter">{editingId ? 'Edit Article' : 'New Draft'}</h1>
          </div>
          {#if editingId}
            <button type="button" on:click={reset} class="text-white/30 hover:text-white text-xs font-bold uppercase transition-colors mb-1">Cancel Edit</button>
          {/if}
        </div>
        ...
        <div class="space-y-2 bg-white/5 p-6 rounded-[2rem] border border-white/5 focus-within:border-blue-500/30 transition-all">
          ...
          <input id="github-repo" bind:value={github_repo} placeholder="e.g. sveltejs/svelte" class="form-input !bg-transparent !border-none !p-0 !px-1 focus:ring-0" />
        ...
        <div class="space-y-2">
          ...
          <div class="editor-container"><div bind:this={editorElement}></div></div>
        </div>

        {#if saveError}<p class="text-red-400 text-sm font-bold bg-red-500/10 p-4 rounded-xl border border-red-500/20">{saveError}</p>{/if}

        <button class="primary-submit-btn" disabled={saving}>{saving ? 'Processing...' : editingId ? 'Update Article' : 'Submit for Review'}</button>
      </form>
          ...
          {#each myPosts as post (post.id)}
            <div class="bg-white/5 p-5 rounded-2xl border border-white/5 hover:border-white/10 transition-all group">
              <div class="flex justify-between items-start mb-3">
                <span class="text-[9px] font-black uppercase px-2 py-1 rounded tracking-tighter {post.status === 'Approved' ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}">
                  {post.status || 'Pending'}
                </span>
                <small class="text-white/20 text-[10px]">{formatDate(post.created_at)}</small>
              </div>
              <h3 class="text-white font-bold text-sm line-clamp-2 mb-4 group-hover:text-blue-400 transition-colors">{post.title}</h3>
...
<style>
  @reference "../../app.css";
  .glass-panel { background: rgba(15, 18, 25, 0.8); backdrop-filter: blur(20px); border: 1px solid rgba(255, 255, 255, 0.08); }
  .form-input { @apply w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-3.5 text-white focus:border-blue-500/50 outline-none transition-all placeholder:text-white/10 text-sm; }
  .editor-container { @apply bg-white/5 border border-white/10 rounded-2xl overflow-hidden; }
  .primary-submit-btn { @apply w-full py-4 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl font-black uppercase tracking-widest text-xs transition-all shadow-lg shadow-blue-600/20 active:scale-[0.98] disabled:opacity-50; }
  .action-btn { @apply flex-1 py-2 rounded-xl text-[10px] font-black uppercase tracking-tighter transition-all; }
